...
- **Logística (`/logistica/etiqueta`, 2026-08-11)**: first feature that produces a **physical document**. Pallet labels at 100×150 mm printed straight from the browser (`@page { size: 100mm 150mm }`, no PDF library) with a server-rendered SVG QR Code pointing at `/logistica/palete/<code>`. Because `olist_products` is flat (1 SKU = 1 row, no variation/grade), each "variation" is a real Olist SKU picked from a `<datalist>`, and the printed label text is derived from `sku` — never from `nome`, which is the marketplace ad title. The label text is frozen in the DB at print time: the label is a document, not a live view. See `docs/logistica-etiquetas.md`.
