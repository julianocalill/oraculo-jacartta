...
- **Persistent shell**: `AppShell` + `SidebarNav` (client, `usePathname`) on every authenticated page (16 tabs in `lib/auth/tabs.ts`). Sidebar badges come from a `badges` map (href → count) that `AppShell` assembles itself: the global alert count (`lib/alert-count.ts`, `unstable_cache` 60s) and the **per-user** Agenda count (`lib/agenda-count.ts`, `React.cache` only — a global `unstable_cache` key would leak counts across users).
