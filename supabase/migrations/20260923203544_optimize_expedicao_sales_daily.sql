-- Expedição: evita que a apuração de vendas caia no statement_timeout do
-- PostgREST e transforme dados reais em zeros na tela.
--
-- A janela padrão de sete dias lia milhares de páginas do heap por três
-- caminhos independentes (pedidos, itens e pacotes). Em cache frio a RPC
-- ultrapassou 15 s em produção. Os índices abaixo cobrem exatamente as
-- colunas usadas pela conciliação e a CTE materializada limita cada leitura a
-- uma passagem pela seleção de pedidos.

create index if not exists shopee_orders_fulfillment_sales_cover_idx
  on public.shopee_orders (pay_time)
  include (id, shop_id, order_sn, synced_at)
  where pay_time is not null
    and upper(coalesce(order_status, '')) not in ('CANCELLED', 'IN_CANCEL', 'UNPAID');

create index if not exists shopee_order_items_fulfillment_cover_idx
  on public.shopee_order_items (order_id)
  include (quantity);

create index if not exists shopee_fulfillment_packages_order_cover_idx
  on public.shopee_fulfillment_packages (shop_id, order_sn)
...
revoke all on function public.oraculo_fulfillment_sales_daily(date, date, bigint)
