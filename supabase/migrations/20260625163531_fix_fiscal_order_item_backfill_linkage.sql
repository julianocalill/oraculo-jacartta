...
as $$
  with linked_invoices as (
    select
      ...
      linked.numero_pedido,
      linked.data_criacao as order_data_criacao,
      linked.payload as order_payload
    from public.oraculo_fiscal_invoices_valid invoices
    cross join lateral (
      select orders.id, orders.numero_pedido, orders.data_criacao, orders.payload
  ...
  unique_orders as (
    select distinct on (linked_invoices.order_id)
      linked_invoices.*
    from linked_invoices
    where (p_after_order_id is null or linked_invoices.order_id > p_after_order_id)
      and not exists (
        select 1
        from public.olist_order_items items
        where items.order_id = linked_invoices.order_id
      )
    order by linked_invoices.order_id, linked_invoices.issued_at, linked_invoices.invoice_id
  )
  select
...
as $$
  ...
  select count(distinct linked_orders.order_id)
  from linked_orders
  where not exists (
    select 1
    from public.olist_order_items items
    where items.order_id = linked_orders.order_id
  );
$$;
...
revoke all on function public.oraculo_fiscal_order_item_backfill_progress(date, date) from public, anon, authenticated;
grant execute on function public.oraculo_fiscal_order_item_backfill_progress(date, date) to service_role;
