-- O export do de-para precisa identificar o anúncio sem depender do SKU do
-- seller (no ML quase nenhum anúncio tem SKU; na Shopee o par item/variação é
-- a identidade real). O cache já guardava o item_id; passa a guardar também o
-- ID da variação (model_id Shopee / variation_id ML; TikTok fica nulo — a
-- fonte é a base de devoluções, que não traz IDs).
-- ATENÇÃO: a função é recriada por inteiro (create or replace); qualquer
-- mudança futura deve partir DESTA versão.

alter table public.oraculo_sku_channel_map_cache
  add column if not exists channel_model_id text;
...
-- A view "select *" precisa ser recriada para enxergar a coluna nova.
drop view if exists public.oraculo_sku_channel_map;
create view public.oraculo_sku_channel_map as
