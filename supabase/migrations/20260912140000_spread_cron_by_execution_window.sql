...
-- média e 239s no pior caso: ele começa em :28 e continua rodando
...
--   - Só 6 dos 48 jobs rodam por mais de 5s. Os outros 42 disparam
--     por pg_net e terminam em 0,04-0,3s.
...
--     disputando as 2 vCPUs com o app.
--   - Pico simulado sobre a semana inteira: 6 jobs simultâneos no domingo
--     10:30, ainda antes de a Giracasa ter crons.
--
-- Recurso disputado (corrigido em 14/09): com
-- `cron.use_background_workers = off`, cada job é uma sessão de cliente e
-- consome `max_connections = 90`, não `max_worker_processes = 6`. O custo de
-- sobrepor jobs pesados é CPU e conexão compartilhadas com o PostgREST.
--
...
begin
  for ajuste in
    select * from (values
      ...
      -- 67s: sai do :42, onde rodava junto com take-rate-cache (99s)
