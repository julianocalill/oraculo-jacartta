...
import {
  calcNetCostByOrigin, calcGiraCasaNetCost, calcPisCofins, calcDifal, icmsRateForUf,
  GIRA_CASA_INTERNAL_ICMS_RATES
} from "./fiscal.js";
...
export function calcOperationFiscalOrder({ operationId, invoiceValue, grossCost, netCost,
  recoverableTaxes, purchaseIcmsRate, origin, destState,
  ...
  // gira-casa-v2 (contador, 17/09/2026): custo líquido de ICMS da compra e de
  // PIS/COFINS; na venda, PIS/COFINS cheio e DIFAL pela tabela do contador.
  const costResult = sp
    ? calcGiraCasaNetCost({ grossTotal: grossCost, netTotal: netCost, recoverableTaxes,
      purchaseIcmsRate, origin, pisCofinsRate })
  ...
  const pisCofins = base == null || cost == null ? null : calcPisCofins({ base, netCost: cost,
    rate: pisCofinsRate, creditEnabled: sp ? false : (pisCofinsCreditEnabled ?? false) });
  ...
  const difal = base == null || icmsRate == null ? null : intrastate ? 0 : calcDifal({ base, destState,
    sourceState: operation.state, origin, explicitAmount: difalOverrideAmount, explicitRate: difalOverrideRate,
    ...(sp ? { internalRates: GIRA_CASA_INTERNAL_ICMS_RATES } : {}) }).amount;
  ...
  return { operationId, profile: operation.profile, ruleVersion: sp ? "gira-casa-v2" : `${operation.profile}-v1`, base,
