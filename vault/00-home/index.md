...
- Sales curve page `/curva-de-venda` is live in code. It lists simple Olist products with `disponivel > 0` and `tipo <> K`, excludes kits, shows product name, last sale date, stock quantity and curve, and groups products into A/B/C by days since last sale. Last sale comes from `olist_order_items.order_data_criacao` by `produto_id`.
