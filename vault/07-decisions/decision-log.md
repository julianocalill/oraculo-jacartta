...
- 2026-07-17: A lógica de sugestão vive em `build-suggestions.ts` por canal e é compartilhada pela página e pela rota de export .xlsx — a planilha não pode divergir da tela por construção (nunca duplicar o cálculo no export).
- 2026-07-17: Explicações das colunas ficam centralizadas em `apps/web/lib/column-hints.ts`; toda coluna calculada nova deve ganhar um `hint` para que ML e Shopee expliquem a métrica com as mesmas palavras.
- 2026-08-11: o relatório operacional de vendas segue Shopee Open Platform → n8n → Evolution API → WhatsApp, sem ler dados do Oráculo. O n8n `Zeptn7GL4bOOsGKj` é o único renovador dos tokens Shopee; o Oráculo recebe uma réplica não bloqueante e suas funções apenas consomem tokens.
- 2026-08-14: Custo do produto passa a ser líquido dos créditos recuperáveis (−9,25% nacional, −11,75% importado), em toda a plataforma — motor fiscal, Shopee, Mercado Livre, devoluções e margem operacional. A regra vive na função `oraculo_net_cost`; kit desconta por componente e o override manual de `/parametros` é bruto. ADR-005.
- 2026-08-14: DIFAL do motor fiscal passa a ser a diferença simples de alíquotas (`base da NF × max(0, interna do destino − interestadual)`), só em operação interestadual, por orientação do contador — sai a base "por dentro" da LC 190/2022. O motor passa a divergir do `vICMSUFDest` impresso na NF de propósito, como já divergia no ICMS. ADR-004.
- 2026-07-17: Regra de layout: filhos diretos do `.workspace` levam `min-width: 0`. Grid items nascem com `min-width: auto` e uma tabela larga estoura o painel, fazendo a página inteira rolar e arrastar a sidebar. Só o `.table-wrap` rola.
