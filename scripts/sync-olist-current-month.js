...
async function listOrderIds(env, monthStart, resetAll, limit) {
  const filters = [
    "select=id",
  ...
  if (!resetAll) {
    filters.push(`data_criacao=lt.${monthStart}`);
  }
...
async function deleteOrders(env, monthStart, resetAll) {
  const batchSize = 1000;
  let deleted = 0;

  while (true) {
    const rows = await listOrderIds(env, monthStart, resetAll, batchSize);
    if (rows.length === 0) {
      break;
    }
...
async function deleteOrderRuns(env, monthStart, resetAll) {
  const filter = resetAll ? "id=not.is.null" : `window_end=lt.${monthStart}`;
  const { response, text } = await supabaseFetch(env, `rest/v1/olist_sync_runs?${filter}`, {
    method: "DELETE",
    headers: {
      Prefer: "return=minimal"
    }
  ...
  if (!response.ok) {
    throw new Error(`Falha ao limpar runs de pedidos (${response.status}): ${text.slice(0, 300)}`);
  }
}
...
async function main() {
  ...
  const monthStart = toLocalIsoDate(firstDayOfMonth(today));
  const todayIso = toLocalIsoDate(today);
  const resetAll = String(process.env.RESET_ALL || "false").toLowerCase() === "true";

  await deleteOrders(env, monthStart, resetAll);
  await deleteOrderRuns(env, monthStart, resetAll);

  runNodeScript(join(repoRoot, "scripts/import-olist-orders-full.js"), {
    ORDER_BACKFILL_START_DATE: monthStart,
    ORDER_BACKFILL_END_DATE: todayIso,
    ORDER_BACKFILL_WINDOW_DAYS: "1"
  });

  runNodeScript(join(repoRoot, "scripts/hydrate-olist-order-details.js"), {
    DETAIL_START_DATE: monthStart,
    DETAIL_END_DATE: todayIso,
