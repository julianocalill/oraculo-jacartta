...
//
// O prompt NÃO lista as receitas: medido contra o servidor real, o
// qwen2.5-coder:7b devolveu `receita: null` mesmo com instrução explícita para
// escolher uma. O estágio determinístico já acerta a receita (7 de 7 nas
// perguntas testadas) e a tela a mostra em destaque — pedir de novo ao modelo
// só alongava o prompt, que na CPU custa segundos.
...
- Só cite nomes que aparecem na lista acima.
- Se a lista não responde a pergunta, diga isso claramente.

Responda SOMENTE com JSON válido, neste formato:
{"resposta":"texto explicando onde achar o dado","objetos":["nome_do_objeto"]}`;
}
...
export async function askOllama(
  ...
  try {
    ...
    return { resposta, objetos, inventados };
  } catch (error) {
