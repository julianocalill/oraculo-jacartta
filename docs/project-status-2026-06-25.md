...
- receita fiscal coberta via pedido e itens: `0,87%`.
...
`scripts/backfill-olist-order-items-for-valid-invoices.js`

Requisitos:

- selecionar somente pedidos vinculados a NFs validas;
- processar apenas pedidos ainda sem itens;
- aceitar inicio, fim, limite, delay, runtime maximo e resume;
- checkpoint persistente;
- retry/backoff para rede, `429` e `5xx`;
- upsert dos itens com payload bruto;
- relatorio de processados, sem itens e erros;
- auditoria automatica depois de cada lote.
